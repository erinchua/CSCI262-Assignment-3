...
def generateOnline(min_val, max_val, mean, std, days):
    ...
    # define the distribution
    dist = stats.gengamma.rvs((max_val-mean)/std, (min_val-mean)/std, loc=mean, scale=std)
    ...
    # get the mean and stdev
    dataMean = statistics.mean(dist)
    dataStdev = statistics.stdev(dist)

    print("Original Data", dist)
    # print("Rounded Data", roundedData)
    print("Mean: ", dataMean)
    print("St.dev: ", dataStdev)


# generateOnline(a, b, mu, sigma, 5)
